    ...
    public sealed class EventSequenceDrawer : PropertyDrawer
    {
        ...
        public static void DoNameGUI(ref Rect area, Context context, int index, string nameLabel)
        {
            ...
            if (index < context.Names.Count)
            {
                ...
                EditorGUI.PropertyField(nameArea, nameProperty, content);
            }
            else
            {
                EditorGUI.BeginChangeCheck();

                EditorGUI.BeginProperty(nameArea, GUIContent.none, context.Names.Property);
                var name = EditorGUI.TextField(nameArea, content, "");
                EditorGUI.EndProperty();
            ...
            string timeLabel, float defaultTime, bool isEndEvent)
        {
            ...
            var timeContent = AnimancerGUI.TempContent(timeLabel,
