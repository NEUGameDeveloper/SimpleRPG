    ...
    partial class AnimancerState
    {
        ...
        private sealed class EventRunner : Key, IUpdatable
        {
            ...
            public static void Acquire(AnimancerState state)
            {
                ...
                WarningType.UnsupportedEvents.Log(state.UnsupportedEventsMessage, state.Root?.Component);
            ...
            {
#if UNITY_ASSERTIONS
                if (_Events != null && !_LoggedEndEventInterrupt)
                {
                    _LoggedEndEventInterrupt = true;
                    WarningType.EndEventInterrupt.Log(
                        $"{callback.Method.DeclaringType.Name}.{callback.Method.Name}" +
                        " was invoked by an End Event, but it did not actually end the animation." +
                        " End Events are triggered every frame after their time has passed for the sake of reliability," +
                        " so in this case it may be necessary to explicitly clear the event or simply use a regular event instead.",
                        _State.Root?.Component);
                }

                if (WarningType.DuplicateEvent.IsDisabled())
                    return;
                ...
                for (int iA = 0; iA < count; iA++)
                {
                    ...
                    for (int iB = iA + 1; iB < count; iB++)
                    {
                        ...
                        if (a == b)
                        {
                            WarningType.DuplicateEvent.Log(
