...
namespace Animancer.Examples.Jobs
{
    ...
    /// <list type="bullet">
    /// <item>This script sets up the job in place of https://github.com/Unity-Technologies/animation-jobs-samples/blob/master/Assets/animation-jobs-samples/Samples/Scripts/Damping/Damping.cs</item>
    /// <item>The <see cref="DampingJob"/> script is almost identical to the original from https://github.com/Unity-Technologies/animation-jobs-samples/blob/master/Assets/animation-jobs-samples/Runtime/AnimationJobs/DampingJob.cs</item>
    /// </list>
    /// Note that the Animation Rigging package has a damping system which is likely better than this example.
    /// </remarks>
    [AddComponentMenu(Strings.ExamplesMenuPrefix + "Jobs - Damping")]
    [HelpURL(Strings.ExampleAPIDocumentationURL + nameof(Jobs) + "/" + nameof(Damping))]
